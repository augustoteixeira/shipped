...
async fn draw_command(state: &State, command: &Command) -> Result<(), StateError> {
...
async fn main() -> std::io::Result<()> {
  ...
  loop {
    ...
    draw_text(
      format!("St: {:?}, min {}", state.game_status, state.min_tokens).as_str(),
      1200.,
      216.,
      32.,
      WHITE,
    ...
    // update
    if (get_time() > seconds + FRAME_TIME) & (state.game_status == GameStatus::Running) {
      seconds += FRAME_TIME;
